...
st.set_page_config(page_title="Pension Planner with Spouse", layout="wide")
...
def accumulate_pot(current_pot, monthly_contrib, annual_return, years):
    """Compound growth pre-retirement."""
    months = int(years * 12)
    bal = float(current_pot)
    monthly_r = annual_return / 12.0
    for _ in range(months):
        bal = bal * (1 + monthly_r) + monthly_contrib
    return bal

def simulate_yearly_schedule(pot, post_growth, retire_age, private_start, inflation, gov_monthly, lump_sum, end_age=100):
    ...
    for age in range(retire_age, end_age + 1):
        ...
        gov_income_year = (gov_monthly*12 if age >= 65 else 0.0)
        total_yearly = total_private_year + gov_income_year
        total_monthly_income = private_monthly + (gov_monthly if age >= 65 else 0.0)
        rows.append({
            "Age": age,
            "PrivateMonthly": round(private_monthly,2),
            "GovMonthly": round(gov_monthly if age>=65 else 0.0,2),
...
st.info("Please fill in all fields. Minimum retirement age is 57. After filling, click 'Generate Pension Table' to see results.")
...
with col1:
    ...
    retire_u = st.number_input("Your retirement age", 57, 100, 65, key="retire_u")
...
with col2:
    ...
    retire_s = st.number_input("Spouse retirement age", 57, 100, 65, key="retire_s")
    ...
    pre_growth_s = st.number_input("Spouse pre-retirement annual growth (%)", 0.0, 20.0, 5.0, key="pre_growth_s") / 100.0
    post_growth_s = st.number_input("Spouse post-retirement annual growth (%)", 0.0, 20.0, 4.0, key="post_growth_s") / 100.0
...
if st.button("Generate Pension Table"):
    ...
    if retire_u < 57 or retire_s < 57:
        ...
        acc_u = accumulate_pot(pot_u, contrib_u, pre_growth_u, years_u)
        lump_u = acc_u * (lump_pct_u / 100.0)
        rem_u = acc_u - lump_u

        years_s = retire_s - age_s
        acc_s = accumulate_pot(pot_s, contrib_s, pre_growth_s, years_s)
        ...
        df_u = simulate_yearly_schedule(rem_u_disp, post_growth_u, retire_u, private_u_disp, inflation_sel, gov_monthly_u_disp, lump_u_disp)
        df_s = simulate_yearly_schedule(rem_s_disp, post_growth_s, retire_s, private_s_disp, inflation_sel, gov_monthly_s_disp, lump_s_disp)
