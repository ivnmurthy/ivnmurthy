...
2. Calculate monthly pension before age 68 (only private pension).
3. Calculate monthly pension after age 68 (private pension + government pension).
4. Fetch GBP to INR conversion rate if residency is India.
5. Ensure pension pot depletion calculation is realistic.
6. Apply conversion rate to final values if residency is India.
7. Show separate entries for self and spouse in monthly pension breakdown.
8. Include respective country currency symbol to avoid confusion.
9. Display results in both GBP (£) and INR (₹) with respective flags.
10. Allow for a percentage withdrawal from private pension pot at retirement.
"""
...
def main():
    ...
    if st.button("Calculate Pension"):
        ...
        st.write(f"💰 **Your Pension Pot**: 🇬🇧 £{user_accumulated_pension:,.2f} / "
                 f"🇮🇳 ₹{user_accumulated_pension_inr:,.2f}")
        st.write(f"💰 **Spouse's Pension Pot**: 🇬🇧 £{spouse_accumulated_pension:,.2f} / "
                 f"🇮🇳 ₹{spouse_accumulated_pension_inr:,.2f}")
        st.write(f"💰 **Total Accumulated Pension**: 🇬🇧 £{total_accumulated_pension:,.2f} / "
        ...
        if residency == "India":
            st.write(f"🛑 **Note**: All amounts have been converted using the current exchange rate "
                     f"of 1 GBP = {conversion_rate} INR.")
